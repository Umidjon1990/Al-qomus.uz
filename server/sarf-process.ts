...
export async function sarfProcess<T>(script:'engine'|'lookup'|'export_pdf',input:unknown):Promise<T> {
 ...
 try { return await new Promise<T>((ok,fail)=>{
  ...
  child.stdout.on('data',chunk=>{output+=chunk;if(output.length>(script==='export_pdf'?2000000:256000)){child.kill();finish(Error('Sarf javobi juda katta'));}});
