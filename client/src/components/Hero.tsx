  ...
  return (
    <section className="sticky top-12 z-40 border-b border-gray-200 bg-white shadow-sm">
      <div className="max-w-4xl mx-auto px-4 py-3 sm:py-4">
        ...
        <div className="relative flex items-center rounded-xl border-2 border-teal-500 bg-white focus-within:border-teal-500 focus-within:ring-2 focus-within:ring-teal-100">
          <Search className="absolute left-4 h-5 w-5 text-teal-600" aria-hidden="true" />
          ...
          {searchTerm && <button type="button" aria-label="Qidiruvni tozalash" onClick={() => setSearchTerm('')} className="absolute right-1 p-3 text-gray-500 hover:text-teal-600"><X className="h-5 w-5" /></button>}
        </div>
      </div>
    </section>
  );
