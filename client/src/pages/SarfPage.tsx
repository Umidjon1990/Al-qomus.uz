...
import { persons, type SarfVerb, type conjugate } from '@shared/sarf';
...
export default function SarfPage({params}:{params?:{id?:string}}) {
 ...
 useEffect(()=>{setSection('past');setVoice('active');setMood(1);},[params?.id]);
 ...
 const detail=useQuery<{verb:SarfVerb;tables:ReturnType<typeof conjugate>}>({queryKey:['sarf-entry',params?.id],enabled:!!params?.id,queryFn:async()=>{const r=await fetch('/api/sarf/'+params?.id);if(!r.ok)throw Error();return r.json();}});
 ...
 return <Layout><div className="max-w-4xl mx-auto px-4 py-6">
   ...
   {!params?.id ? <>
     <p className="text-gray-600 mb-4">Sulosiy mujarradning solim fe’llari va Af’ala solim. Moziy, muzori’ yoki o‘zbekcha ma’no bilan qidiring.</p>
     ...
     {list.data&&<><p className="text-sm text-gray-500 mb-3">{list.data.total} ta yozuvda ushbu bosqich uchun yetarli ma’lumot bor. {q&&`${list.data.count} ta natija.`} {list.data.count>60&&'Dastlabki 60 tasi ko‘rsatilmoqda.'}</p>
     ...
     <button onClick={()=>navigate('/sarf')} className="text-sm text-gray-600 mb-4">← Boshqa fe’l qidirish</button>
     ...
     {v&&data&&<>
       <div className="bg-white border rounded-xl p-5 mb-5"><h2 className="font-arabic text-3xl leading-loose" dir="rtl">{v.past} — {v.present}</h2><p>{v.meaning}</p><p className="mt-2 text-sm text-gray-500">{v.form==='I'?`Sulosiy mujarrad · ${v.bab}-bob`:'Af’ala bobi'} · Solim · Ildiz: <span dir="rtl">{v.root}</span> · Manba: {v.source}</p>
       {v.masdar&&<p className="mt-2">Masdar (lug‘atdan): <span className="font-arabic text-xl" dir="rtl">{v.masdar}</span></p>}</div>
       <div className="flex flex-wrap gap-2 mb-3" aria-label="Zamon">{tabs.map(([id,label])=><button key={id} className={button(section===id)} aria-pressed={section===id} onClick={()=>setSection(id)}>{label}</button>)}</div>
